<aside class="main-sidebar">
    <section class="sidebar">
        <div class="user-panel animated fadeIn">
            <div class="text-center image">
                <img data-src="{{ Avatar::create(CRUDBooster::myName())->toBase64() }}" class="img-circle lazy-img mt-2" alt="{{ cbLang('user_image') }}"/>
                <h4 class="mb-0">{{ CRUDBooster::myName() }}</h4>
                <span class="text-muted">{{ CRUDBooster::myPrivilegeName() }}</span>
            </div>
        </div>
        ...
        <div class='main-menu'>
            <ul class="sidebar-menu">
                ...
                @if($dashboard)
                    <li data-id='{{$dashboard->id}}' class="{{ (Request::is(config('crudbooster.ADMIN_PATH'))) ? 'active' : '' }}">
                        <a href='{{CRUDBooster::adminPath()}}' class='{{($dashboard->color)?"text-".$dashboard->color:""}}'>
                            <i class='fa fa-home'></i> <span>{{cbLang("text_dashboard")}}</span>
                        </a>
                    </li>
                @endif
                ...
                @foreach(CRUDBooster::sidebarMenu() as $menu)
                    <li data-id='{{$menu->id}}' class='{{(!empty($menu->children))?"treeview":""}} {{ (Request::is($menu->url_path."*"))? $already = true . " active":""}}'>
                        ...
                        @if(!empty($menu->children))
                            <ul class="treeview-menu">
                                @foreach($menu->children as $child)
                                    <li data-id='{{$child->id}}' class='{{(Request::is($child->url_path .= !Str::endsWith(Request::decodedPath(), $child->url_path) ? "/*" : "")) ? (!$already ? "active" : "") : ""}}'>
                ...
                @if(CRUDBooster::isSuperadmin())
                    ...
                    <li class="{{ (Request::is(config('crudbooster.ADMIN_PATH').'/logs*')) ? 'active' : '' }}">
                        <a href='{{Route("LogsControllerGetIndex")}}'>
                            <i class='fa fa-flag'></i> <span>{{ cbLang('Log_User_Access') }}</span>
                        </a>
                    </li>
                @endif
            </ul>
