...
use crocodicstudio\crudbooster\export\ChunkArrayExport;
use crocodicstudio\crudbooster\import\DefaultImportXls;
use CRUDBooster;
...
class CBController extends Controller
{
    ...
    public function cbLoader()
    {
        ...
        $this->data['modal_index_html'] = $this->modal_index_html;
    ...
    public function getIndex()
    {
        ...
        foreach ($data['result'] as $row) {
            ...
            if ($this->button_bulk_action && (($this->button_delete && CRUDBooster::isDelete()) || $this->button_selected)) {
            ...
            foreach ($columns_table as $col) {
                ...
                if (isset($col['image'])) {
                    ...
                    if ($result) {
                        ...
                        $value = "<a data-lightbox='roadtrip' rel='group_{{$table}}' title='$label: $title' href='".asset('assets/images/no-image.jpg')."'><img width='40px' height='40px' src='".asset('assets/images/no-image.jpg')."'/></a>";
                    }
                }
        ...
        $data['result'] = $data['result']->onEachSide(0);
    ...
    public function getDownloadRawCsv()
    {
        $response = $this->getIndex()->getData();
        $response['limit'] = 0;
        $response['raw'] = true;
        return Excel::download(new DefaultExportXls($response), "Tabel " . $response['page_title'] . " Raw CSV - " . time() . ".csv");
    }
        ...
        try {
            switch ($filetype) {
                case "pdf":
                    ...
                    break;
                case 'xls':
                    return Excel::download(new DefaultExportXls($response), $filename ." - ". time() . ".xls");
                break;
                case 'csv':
    ...
    public function getMasterData()
    {
        ...
        if (isset($table) && Schema::hasTable($table)) {
            $rows = DB::table($table)->get()->map(function ($q) {
                return collect($q)->except(['created_at', 'updated_at', 'password', 'photo']);
            })->toArray();
            if (collect($rows)->count() > 0) {
                $table_columns = collect(DB::getSchemaBuilder()->getColumnListing($table))->filter(function ($q) {
                    return !in_array($q, ['created_at', 'updated_at', 'password', 'photo']);
                })->toArray();

                $new = array();
                $new = collect($new)->push($table_columns)->merge($rows);
                $new = $new->toArray();

                $table = str_replace("cms_", "", $table);
                $filename = "Master Data " . ucwords($table) . " " . time();
                alert()->html(sprintf('Berhasil memproses<br>%s', $filename), 'success');
                return Excel::download(new ChunkArrayExport($new), $filename.".csv");
            // return redirect(CRUDBooster::mainPath('import-data')->with('success', sprintf('Berhasil memproses %s', $filename));
            } else {
                return redirect()->back()->with('warning', 'Tidak ada data pada tabel yang minta');
            }
        }
        return redirect()->back()->with('warning', 'Terjadi kesalahan. Mohon hubungi Administrator');
    }
...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function getImportData()
    {
        ...
        if (request('file') && ! request('import')) {
            ...
            $rows = Excel::toArray(new DefaultImportXls, $file);
            $countRows = ($rows) ? collect($rows)->count() : 0;
    ...
    public function postDoImportChunk()
    {
        ...
        $rows = Excel::toArray(new DefaultImportXls, $file)[0];
        ...
        $validity = [];
        $errors = null;
        foreach ($this->data_inputan as $in) {
            if ($in['validation'] == null) {
                continue;
            }
            $validity[$in['name']] = $in['validation'];
        }
        ...
        foreach ($rows as $x => $row) {
            ...
            $arr = [];
            foreach ($row as $i => $list) {
                $arr = collect($arr)->merge([$rows[0][$i] => $list]);
            }

            $arr = $arr->toArray();

            foreach ($arr as $k => $x2) {
                if (strpos($k, 'solution_id') !== false) {
                    $solution = $this->getSolution($x2);
                    $arr[$k] = $solution;
                }

                if (strpos($k, 'province_') !== false) {
                    $province = $this->recCheck($x2, 'provinces');
                    $arr[$k] = $province;
                }

                if (strpos($k, 'city_') !== false) {
                    $city = $this->recCheck($x2, 'cities', 'province_id', $province);
                    $arr[$k] = $city;
                }

                if (strpos($k, 'district_') !== false) {
                    $district = $this->recCheck($x2, 'districts', 'city_id', $city);
                    $arr[$k] = $district;
                }
            }

            if ($arr['province_id'] == null || $arr['city_id'] == null || $arr['district_id'] == null) {
                return response()->json(['status' => false, 'message' => json_encode($arr)]);
            }

            $validator = Validator::make($arr, $validity);

            if ($validator->fails()) {
                $list_data = json_encode($row);
                $errors .= "<b>Error baris {$x} | Data : {$list_data}</b><br>";
        ...
        if ($errors != null) {
            ...
            try {
                DB::table($this->table)->insert($new_arr);
                ...
                return response()->json(['status' => false, 'message' => json_encode($x->getMessage())]);
            }
    ...
    public function getSolution($str)
    {
        return DB::table('solutions')->where('name', $str)->first()->id;
    }

    protected $try = 0;
    public function recCheck($str, $table, $parent_name = null, $parent_id = null)
    {
        $contain = strpos($str, "Kabupaten ") !== false;
        $str = str_replace("Kabupaten ", "", $str);

        $check = DB::table($table)
        ->where(function ($q) use ($parent_name, $parent_id, $contain, $str) {
            if ($this->try == 0) {
                $q->where('name', $str);
            } else {
            ...
            if ($parent_name && $parent_id) {
                $q->where($parent_name, $parent_id);
            }

            if ($contain) {
                $q->where('name', 'NOT LIKE', '%Kota%');
            }
        })
            ->get();
        if ($check->count() > 1) {
            ...
            if ($this->try == 0) {
                $this->try += 1;
                $this->recCheck($str, $table, $parent_name, $parent_id);
            } else {
                return null;
            }
        }
